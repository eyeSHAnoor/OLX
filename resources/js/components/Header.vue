        ...
        <div class="w-full bg-slate-100">
            <div class="max-w-9/11 mx-auto px-4 py-4 flex items-center justify-between">
                ...
                <div class="hidden md:flex items-center gap-8">
                    <div v-if="user" class="flex items-center gap-5 relative">
                        ...
                        <div class="relative" v-click-outside="() => showDropdown = false">
                            ...
                            <transition enter-active-class="transition duration-200 ease-out"
                                ...
                                <div v-if="showDropdown"
                                    class="absolute right-0 mt-3 w-md bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-50">
                                    <!-- User Info Header -->
                                    <div class="px-4 py-3 border-b border-gray-100">
                                        <p class="text-lg font-semibold text-gray-900">{{ user.name || 'User Name' }}
                                        </p>
                                        <p class="text-md text-gray-500 mt-0.5">{{ user.email || 'user@example.com' }}
                                        </p>
                                    </div>
                                    ...
                                    <div class="py-1">
                                        <Link :href="route('user.profile', user.id)"
                                            class="flex items-center gap-3 px-4 py-2.5 text-lg text-gray-700 hover:text-brand-blue transition-colors">
                                            <Icon icon="mdi:account-outline" class="text-2xl text-gray-500" />
                                            <span>Public Profile</span>
                                        </Link>
                                        <Link href="/orders"
                                            class="flex items-center gap-3 px-4 py-2.5 text-lg text-gray-700 hover:text-brand-blue transition-colors">
                                            <Icon icon="mdi:package-variant" class="text-2xl text-gray-500" />
                                            <span>My Orders</span>
                                        </Link>
                                        <Link href="/favourites"
                                            class="flex items-center gap-3 px-4 py-2.5 text-lg text-gray-700 hover:text-brand-blue transition-colors">
                                            <Icon icon="mdi:heart-outline" class="text-2xl text-gray-500" />
                                            <span>My Favourites</span>
                                        </Link>
                                        <Link href="/orders"
                                            class="flex items-center gap-3 px-4 py-2.5 text-lg text-gray-700 hover:text-brand-blue transition-colors">
                                            <Icon icon="mdi:package-variant-closed" class="text-2xl text-gray-500" />
                                            <span>My Orders</span>
                                            <span
                                        ...
                                        <Link href="/ads"
                                            class="flex items-center gap-3 px-4 py-2.5 text-lg text-gray-700 hover:text-brand-blue transition-colors">
                                            <Icon icon="mdi:bullhorn-outline" class="text-2xl text-gray-500" />
                                            <span>My Ads</span>
                                        </Link>
                                        <div class="border-t border-gray-100 my-1"></div>
                                        <Link :href="route('subscriptions.index')"
                                            class="flex items-center gap-3 px-4 py-2.5 text-lg text-gray-700 hover:bg-gray-50 transition-colors">
                                            <Icon icon="mdi:crown-outline" class="text-2xl text-yellow-500" />
                                        ...
                                        <Link href="/settings"
                                            class="flex items-center gap-3 px-4 py-2.5 text-lg text-gray-700 hover:bg-gray-50 transition-colors">
                                            <Icon icon="mdi:cog-outline" class="text-2xl text-gray-500" />
                                            <span>Settings</span>
                                        </Link>
                                        <div class="border-t border-gray-100 my-1"></div>
                                        <Link href="/logout" method="post" as="button"
                                            class="w-full flex items-center gap-3 px-4 py-2.5 text-lg text-red-600 hover:bg-red-50 transition-colors">
                    ...
                    <div class="rounded-full p-[5px] bg-gradient-to-r from-brand-orange via-brand-teal to-brand-blue">
                        <button
                            class="flex items-center gap-1 rounded-full px-6 py-2 text-xl font-bold bg-white hover:shadow-md transition-shadow">
                            <Icon icon="mdi:plus" />
                            SELL
            ...
            <div v-if="mobileMenuOpen" class="md:hidden px-4 pb-6 space-y-4 bg-gray-50 rounded-b-xl shadow-inner">
                <!-- Mobile menu content same as above (optional repeat for brevity) -->
            </div>
        </div>
            ...
            <div
                ...
                <div class="relative w-full md:w-1/3">
                    <div
                        ...
                        <Icon icon="mdi:map-marker-outline" class="text-2xl text-brand-blue" />
                ...
                <div class="flex flex-1 relative w-full">
                    ...
                    <button @click="performSearch"
                        class="bg-brand-blue -ml-4 px-6 text-white rounded-r-lg flex items-center hover:bg-yellow-600 transition-colors">
...
const userSelectedCity = ref(!!localStorage.getItem('selectedCity')) // track manual change
