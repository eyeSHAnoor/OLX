...
const notifications_count = computed(() => page.props.notifications_count);
...
const mainNavItems: NavItem[] = computed(() => [
    {
        ...
        isActive: route().current('home') || route().current('dashboard'),
    },

    // {
    //     visible: hasRole(['admin', 'manager']),
    //     title: 'Registrations',
    //     icon: 'fluent-mdl2:open-enrollment',
    //     isActive: route().current('enrollments.*'),
    //     children: [
    //         { title: 'New Registration', icon: 'lucide:book-text', href: route('enrollments.create'), visible: true },
    //         { title: 'All Registrations', icon: 'carbon:category', href: route('enrollments.index'), visible: true },
    //     ],
    // },
