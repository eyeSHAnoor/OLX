    ...
    <AppContainer>
        ...
        <PageHeading>
            ...
            <template #links>
                ...
                <AppButton label="Add Permissions" icon="radix-icons:plus-circled"
                    @click="router.get(route('subscription-permissions.index'))" class="" />
            </template>
        </PageHeading>
                    ...
                    <template #duration_days-cell="{ row }">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium" :class="{
                            ...
                            'bg-green-100 text-green-800':
                                row.original.duration_days > 7 && row.original.duration_days <= 30,
        ...
        <PlanForm :plan="selectedItem" v-model="showModal" :permissions="permissions" />
    </AppContainer>
</template>
