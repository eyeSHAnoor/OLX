...
<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900 mt-1">{{ stats.total_visited }}</p>
                    </div>
        ...
        <div class="flex items-center justify-between flex-wrap gap-3 mb-4">
            <div class="flex items-center gap-4">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900">Referral Network</h3>
                    <span class="text-sm text-gray-500">({{ allReferrals.length }} members)</span>
                </div>
            ...
            <div class="flex gap-2">
                ...
                <div class="relative">
                    <input v-model="searchQuery" type="text" placeholder="Search members..."
                        class="pl-9 pr-4 py-1.5 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" />
        ...
        <!-- Tree View -->
        <div v-if="viewMode === 'tree'" class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-6">
                <!-- Empty State -->
                <div v-if="tree.assignees.length === 0" class="text-center py-12">
                    ...
                    <p class="mt-4 text-lg font-medium text-gray-900">No referrals yet</p>
                    <p class="mt-1 text-sm text-gray-500">
                        Share your referral link to build your network
                    </p>
                    <div
                        class="mt-4 inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors cursor-pointer">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div v-else class="space-y-3">
                    <template v-for="node in tree.assignees" :key="node.user.id">
                        <div class="relative pl-8 border-l-2 border-gray-200 ml-6">
                            <div class="absolute -left-1.5 top-6 w-3 h-3 bg-gray-200 rounded-full"></div>
                            <div class="space-y-3">
                                <div class="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer"
                                    @click="toggleNode(node.user.id)">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-3">
                                            <div class="flex items-center">
                                                <svg class="w-5 h-5 text-gray-600 transition-transform duration-200"
                                                    :class="{ 'rotate-180': expandedNodes.has(node.user.id) }"
                                            ...
                                            <div class="w-10 h-10 rounded-full flex items-center justify-center overflow-hidden"
                                                :class="node.user.profile_image ? '' : 'bg-gray-100'">
                                            ...
                                            <div>
                                                <div class="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    <span v-if="node.total_downline > 0" class="text-xs text-gray-500">
                                                        ({{ node.total_downline }} downline)
                                                    </span>
                                                </div>
                                                <p class="text-sm text-gray-500">{{ node.user.email }}</p>
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            <button v-if="node.user.referral_code"
                                                ...
                                                class="text-sm text-gray-600 hover:text-gray-800 px-2 py-1 rounded hover:bg-gray-50 transition-colors">
                                ...
                                <div v-if="expandedNodes.has(node.user.id) && node.assignees.length > 0"
                                    class="ml-8 space-y-2 border-l-2 border-gray-200 pl-4">
                                    <div v-for="child in node.assignees" :key="child.user.id" class="relative pl-4">
                                        <div class="absolute -left-1.5 top-3 w-3 h-3 bg-gray-200 rounded-full"></div>
                                        <div
                                            ...
                                            <div class="flex items-center justify-between">
                                                <div class="flex items-center gap-3">
                                                    <div class="w-8 h-8 rounded-full flex items-center justify-center overflow-hidden"
                                                        :class="child.user.profile_image ? '' : 'bg-gray-100'">
                                                    ...
                                                    <div>
                                                        <div class="flex items-center gap-2 flex-wrap">
                                                            ...
                                                            <span v-if="child.total_downline > 0"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr v-for="user in filteredReferrals" :key="user.id" class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="w-8 h-8 rounded-full flex items-center justify-center mr-3 overflow-hidden"
                                        :class="user.profile_image ? '' : 'bg-gray-100'">
