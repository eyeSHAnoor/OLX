<template>
  <OlxLayout>
    <TopCategoriesBar />
    <div class="max-w-full px-2 md:max-w-8/10 mx-auto space-y-12 pb-20" :class="theme.bg">
      <!-- Loading State -->
      <div v-if="!ad" class="flex items-center justify-center min-h-[60vh]">
        <div class="text-center">
          <Icon icon="lucide:loader-2" class="size-6 sm:size-8 animate-spin mx-auto mb-3" :class="theme.icon" />
          <p class="text-sm" :class="theme.textMuted">Loading ad details...</p>
        </div>
      </div>

      <div v-else class="container mx-auto px-3 sm:px-4 py-4 sm:py-6 max-w-7xl">
        <!-- Breadcrumb - Compact -->
        <nav class="flex items-center gap-1.5 text-xs sm:text-sm mt-3 flex-wrap" :class="theme.textMuted">
          <button @click="goBack"
            class="inline-flex items-center gap-1 px-3 py-2 mb-2 rounded-md border text-sm transition"
          ...
          <Icon icon="lucide:chevron-right" class="size-3 sm:size-3.5 flex-shrink-0" />
          <span class="font-medium truncate max-w-[150px] sm:max-w-[300px]" :class="theme.text">{{ ad.ad_title }}</span>
        </nav>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          ...
          <div class="lg:col-span-2 space-y-4 sm:space-y-5">
            <!-- Image Gallery -->
            <div class="rounded-lg sm:rounded-xl shadow-sm overflow-hidden" :class="theme.card">
              <!-- Main Image -->
              <div class="relative aspect-[16/9]" :class="theme.imageBg">
                ...
                <div v-else class="flex items-center justify-center h-full">
                  <Icon icon="lucide:image" class="size-8 sm:size-10" :class="theme.textMuted" />
                </div>

                <!-- Image Navigation -->
                <button v-if="ad.images?.length > 1" @click="prevImage"
                  class="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 p-1.5 sm:p-2 rounded-full shadow-md transition-all"
                  :class="theme.card">
                  <Icon icon="lucide:chevron-left" class="size-4 sm:size-5" :class="theme.text" />
                </button>
                <button v-if="ad.images?.length > 1" @click="nextImage"
                  class="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 p-1.5 sm:p-2 rounded-full shadow-md transition-all"
                  :class="theme.card">
                  <Icon icon="lucide:chevron-right" class="size-4 sm:size-5" :class="theme.text" />
                </button>
                ...
                <button @click="toggleFavorite"
                  class="absolute top-2 sm:top-4 right-2 sm:right-4 p-1.5 sm:p-2 rounded-full shadow-md hover:scale-110 transition-transform"
                  :class="theme.card" :disabled="isFavoriteLoading">
                  <Icon :icon="isFavorited ? 'mdi:heart' : 'lucide:heart'" class="size-4 sm:size-5 transition-colors"
                    :class="[
              ...
              <!-- Thumbnail Strip -->
              <div v-if="ad.images?.length > 1" class="p-3 sm:p-4 border-t" :class="theme.border">
                <div class="flex gap-1.5 sm:gap-2 overflow-x-auto pb-1 sm:pb-2">
                  <button v-for="(image, index) in ad.images" :key="image.id" @click="selectImage(index)"
            ...
            <!-- Ad Details Card -->
            <div class="rounded-lg sm:rounded-xl shadow-sm p-4 sm:p-5 lg:p-6" :class="theme.card">
              <div class="flex items-start justify-between gap-3 mb-3 sm:mb-4">
                <h1 class="text-md sm:text-lg lg:text-xl font-semibold mb-3 sm:mb-4" :class="theme.text">
                  {{ ad.ad_title }}
                </h1>
                ...
                <div class="flex items-center gap-2 sm:gap-3">
                  ...
                  <!-- Save Button -->
                  <button @click="toggleFavorite"
                    class="group relative flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 border-2 rounded-lg transition-all duration-200 hover:scale-105 active:scale-95 shadow-sm hover:shadow-md"
                    :class="[
                      isFavorited
                  ...
                  <div class="relative" @click.stop>
                    <button @click="toggleShareDropdown"
                      class="group relative flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 border-2 border-gray-200 rounded-lg transition-all duration-200 hover:scale-105 active:scale-95 shadow-sm hover:shadow-md hover:border-blue-200 hover:bg-blue-50">
                      <Icon icon="lucide:share-2" class="size-4 sm:size-5 text-gray-600 group-hover:text-blue-600" />
                      <span
                    ...
                    <div v-if="showShareDropdown"
                      class="absolute right-0 mt-2 w-56 rounded-lg shadow-xl border py-1 z-50 animate-in slide-in-from-top-2"
                      ...
                      <button @click="copyAdLink"
                        ...
                        <Icon icon="lucide:copy" class="size-4" :class="theme.textMuted" />
                        <span>Copy link</span>
                      </button>
              ...
              <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 sm:mb-5">
                <div class="flex items-center gap-2 sm:gap-3 flex-wrap">
                  ...
                  <span class="px-2 py-0.5 bg-green-100 text-green-700 rounded-full text-xs sm:text-sm font-medium">
                    {{ ad.is_featured ? "Featured" : "Regular" }}
                  </span>
                </div>
                <span class="text-xs sm:text-sm" :class="theme.textMuted">
                  Posted: {{ formatDate(ad.created_at) }}
                </span>
              </div>

              <!-- Key Details Grid -->
              <div class="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 p-3 sm:p-4 rounded-lg mb-4 sm:mb-5"
                :class="theme.bgLight">
                <div>
                  <p class="text-xs mb-0.5" :class="theme.textMuted">
                ...
                <div>
                  <p class="text-xs mb-0.5" :class="theme.textMuted">Category</p>
                  <p class="text-sm font-medium" :class="theme.text">
                    {{ ad.category?.name || "Not specified" }}
                  </p>
                </div>
                <div>
                  <p class="text-xs mb-0.5" :class="theme.textMuted">Location</p>
                  <p class="text-sm font-medium" :class="theme.text">
                ...
                <div>
                  ...
                  <p class="text-sm font-medium" :class="theme.text">{{ ad.seller_name }}</p>
                </div>
              </div>

              <!-- Description -->
              <div class="mb-4 sm:mb-5">
                <h2 class="text-base sm:text-md font-semibold mb-2 sm:mb-3" :class="theme.text">
                  Description
                </h2>
                <p class="text-xs sm:text-sm leading-relaxed whitespace-pre-line" :class="theme.textMuted">
                  {{ ad.description }}
                </p>
              </div>

              <!-- Features/Specifications -->
              <div v-if="ad.features?.length" class="border-t pt-4 sm:pt-5" :class="theme.border">
                <h2 class="text-base sm:text-lg font-semibold mb-3 sm:mb-4" :class="theme.text">
                  Features & Specifications
                </h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                  <div v-for="feature in ad.features" :key="feature.id"
                    class="flex items-start gap-2 p-2 sm:p-3 rounded-lg" :class="theme.bgLight">
                    <Icon icon="lucide:check-circle" class="size-4 sm:size-5 text-green-500 flex-shrink-0 mt-0.5" />
                    <div>
                      <p class="text-xs" :class="theme.textMuted">{{ feature.name }}</p>
                      <p class="text-sm font-medium" :class="theme.text">
                        {{ feature.value }}
                      </p>
                    </div>
              ...
              <!-- Search Keywords -->
              <div v-if="ad.search_keywords?.length" class="border-t pt-4 sm:pt-5 mt-4 sm:mt-5" :class="theme.border">
                <h2 class="text-base sm:text-md font-semibold mb-2 sm:mb-3" :class="theme.text">
                  Related Keywords
                </h2>
                <div class="flex flex-wrap gap-1.5 sm:gap-2">
                  <span v-for="keyword in ad.search_keywords.slice(0, 15)" :key="keyword"
                    class="px-2 py-1 sm:px-2.5 sm:py-1.5 rounded-full text-xs transition-colors cursor-default"
                    :class="[theme.badge, theme.text]">
                    {{ keyword }}
                  </span>
                  <span v-if="ad.search_keywords.length > 15" class="px-2 py-1 text-xs" :class="theme.textMuted">
          ...
          <div class="space-y-4 sm:space-y-5">
            <!-- Seller Card -->
            <div class="rounded-lg sm:rounded-xl shadow-sm p-4 sm:p-5" :class="theme.card">
              <h2 class="text-base sm:text-md font-semibold mb-3 sm:mb-4" :class="theme.text">
                Seller Information
              </h2>

              <Link :href="route('user.profile', ad?.user?.id)" class="flex items-center gap-3 mb-4 sm:mb-5">
                <div
                  class="size-12 sm:size-14 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                  <Icon icon="lucide:user" class="size-6 sm:size-7 text-primary" />
                </div>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    <p class="font-medium text-sm sm:text-md" :class="theme.text">{{ ad?.user?.name }}</p>
                    <div v-if="$hasPlanPermission('pro_batch', ad?.user)" class="flex-shrink-0">
                      <img src="/images/pro.png" alt="Pro" class="h-7 w-auto" />
                    </div>
                    <div v-if="$hasPlanPermission('premium_batch', ad?.user)" class="flex-shrink-0">
              ...
              <div class="space-y-2 sm:space-y-2.5">
                ...
                <button @click="copyPhoneNumber"
                  class="flex gap-1.5 w-full p-2.5 sm:p-3 border rounded-lg transition-colors text-sm"
            ...
            <div class="bg-blue-50 rounded-lg sm:rounded-xl p-4 sm:p-5">
              ...
              <ul class="space-y-1.5 text-xs sm:text-sm text-blue-800">
                <li class="flex items-start gap-1.5">
                  <Icon icon="lucide:check" class="size-3.5 sm:size-4 text-blue-600 mt-0.5 flex-shrink-0" />
        ...
        <!-- Ad Rating Section -->
        <div class="rounded-lg sm:rounded-xl shadow-sm p-4 sm:p-5 lg:p-6 mt-6" :class="theme.card">
          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 mb-6 pb-6 border-b"
            :class="theme.border">
            <div class="flex flex-col items-center">
              <span class="text-4xl sm:text-5xl font-bold" :class="theme.text">{{
                adAvgRating.toFixed(1)
              }}</span>
              <span class="text-xs mt-1" :class="theme.textMuted">out of 5</span>
            </div>
            ...
            <div class="flex-1 w-full">
              <div class="flex items-center gap-2 mb-3">
                ...
                <span class="text-sm" :class="theme.textMuted">Average Rating</span>
              </div>

              <div class="space-y-2">
                <div v-for="star in [5, 4, 3, 2, 1]" :key="star" class="flex items-center gap-2 text-xs">
                  <span class="w-8" :class="theme.textMuted">{{ star }} star</span>
                  <div class="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div class="h-full bg-yellow-400 rounded-full" :style="{ width: `${getRatingPercentage(star)}%` }">
                    </div>
                  </div>
                  <span class="w-8" :class="theme.textMuted">{{ getRatingCount(star) }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- Your Rating Section -->
          <div class="mb-6">
            <h3 class="text-sm font-medium mb-3" :class="theme.text">Rate this ad</h3>

            <div v-if="page.props.auth?.user && page.props.auth.user.id !== ad?.user_id" class="rounded-lg p-4"
              :class="theme.bgLight">
              <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                <div class="flex gap-1">
                  <button v-for="star in 5" :key="star" @click="submitRating(star)" @mouseover="hoverRating = star"
                    @mouseleave="hoverRating = 0" class="focus:outline-none transition-all duration-200 hover:scale-110"
                ...
                <div class="flex items-center gap-3">
                  <span v-if="userCurrentRating > 0" class="text-sm" :class="theme.textMuted">
                    You rated this
                    <span class="font-medium" :class="theme.text">{{ userCurrentRating }}/5</span>
                  </span>
                  <span v-else class="text-sm" :class="theme.textMuted">Click a star to rate</span>

                  <Icon v-if="isSubmitting" icon="lucide:loader-2" class="size-5 animate-spin" :class="theme.icon" />
                </div>
              </div>
            </div>

            <div v-else-if="!page.props.auth?.user" class="rounded-lg p-4 text-center" :class="theme.bgLight">
            ...
            <div v-else-if="page.props.auth?.user && page.props.auth.user.id === ad?.user_id"
              class="rounded-lg p-4 text-center" :class="theme.bgLight">
              <p class="text-sm" :class="theme.textMuted">
          ...
          <div v-if="recentRatings.length > 0">
            <div class="flex items-center justify-between mb-3">
              <h3 class="text-sm font-medium" :class="theme.text">Recent ratings</h3>
            ...
            <div class="space-y-3">
              <div v-for="rating in recentRatings.slice(0, 3)" :key="rating.id"
                class="flex items-start justify-between p-3 rounded-lg" :class="theme.bgLight">
                <div class="flex-1">
                  <div class="flex items-center gap-2 mb-1">
                    <Link :href="route('user.profile', rating.rater?.id)"
                      class="font-medium text-sm hover:text-primary transition-colors" :class="theme.text">
                      {{ rating.rater?.name || "Anonymous" }}
                    </Link>
                    <span class="text-xs" :class="theme.textMuted">•</span>
        ...
        <div v-if="similarAds?.length" class="mt-8 sm:mt-10 lg:mt-12">
          <h2 class="text-lg sm:text-xl lg:text-2xl font-semibold mb-4 sm:mb-5" :class="theme.text">
            Similar Ads
          </h2>
    ...
    <Teleport to="body">
      <div v-if="lightboxOpen" class="fixed inset-0 z-[9999] bg-black/95 flex items-center justify-center"
        ...
        <div class="w-full h-full flex items-center justify-center overflow-hidden touch-none" @mousedown="startDrag"
          @mousemove="onDrag" @mouseup="stopDrag" @mouseleave="stopDrag" @touchstart="handleTouchStart"
          @touchmove.prevent="handleTouchMove" @touchend="handleTouchEnd" @dblclick="toggleZoom" @wheel.prevent="
            (e) => {
          ...
          <img :src="`/storage/${ad.images[lightboxIndex]?.path}`" :alt="ad.ad_title"
            class="max-w-[95vw] max-h-[95vh] object-contain transition-transform duration-200 select-none" :style="{
