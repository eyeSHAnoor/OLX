<template>
    <OlxLayout>
        ...
        <section v-if="genericBanners.length" class="relative h-[200px] md:h-[300px] lg:h-[400px] overflow-hidden"
            :class="theme.bg">
            ...
            <button v-if="genericBanners.length > 1" @click="prevSlide"
                class="absolute left-4 top-1/2 -translate-y-1/2 z-20 rounded-full p-2 md:p-3 shadow-md transition"
                :class="theme.card">
                <Icon icon="mdi:chevron-left" class="text-xl md:text-2xl" :class="theme.text" />
            </button>
            <button v-if="genericBanners.length > 1" @click="nextSlide"
                class="absolute right-4 top-1/2 -translate-y-1/2 z-20 rounded-full p-2 md:p-3 shadow-md transition"
                :class="theme.card">
                <Icon icon="mdi:chevron-right" class="text-xl md:text-2xl" :class="theme.text" />
            </button>
        ...
        <section class="max-w-full md:max-w-8/10 mx-auto px-4 md:px-3 sm:px-4 py-4 md:py-6" :class="theme.bg">
            <div class="py-3">
                <button @click="goBack"
            ...
            <div class="lg:hidden mb-3">
                <button @click="showMobileFilters = !showMobileFilters"
                    class="w-full flex items-center justify-between p-3 rounded-lg shadow-sm border transition-colors"
                    :class="[theme.card, theme.border, theme.borderHover]">
                    <span class="text-sm font-medium flex items-center gap-1.5" :class="theme.text">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" :class="theme.icon">
                    ...
                    <svg class="w-4 h-4 transition-transform"
                        :class="[theme.textMuted, { 'rotate-180': showMobileFilters }]" fill="none"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-5">
                ...
                <aside class="lg:col-span-1 space-y-4"
                    ...
                    <div class="lg:hidden flex items-center justify-between mb-3">
                        <h2 class="font-semibold text-base" :class="theme.text">Filters</h2>
                        <button @click="showMobileFilters = false" class="p-1.5 rounded-lg" :class="theme.hover">
                    ...
                    <div class="rounded-lg shadow-sm p-4" :class="theme.card">
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="font-medium text-sm flex items-center gap-1.5" :class="theme.text">
                            ...
                            <button @click="showAllCategories = !showAllCategories" v-if="categories?.length > 5"
                                class="text-xs" :class="theme.textAccent">
                    ...
                    <div class="rounded-lg shadow-sm p-4" :class="theme.card" v-if="brands?.length">
                        ...
                        <div class="space-y-1 max-h-60 overflow-y-auto">
                            <div v-for="brand in filteredBrands" :key="brand.id" @click="toggleBrand(brand.id)" :class="[
                                ...
                                theme.hover,
                                selectedBrands.includes(brand.id) ? 'bg-brand-blue/10 text-brand-blue' : theme.text
                            ]">
                        ...
                        <button v-if="brands.length > 10" @click="showAllBrands = !showAllBrands"
                            class="mt-2 text-xs flex items-center gap-1" :class="theme.textAccent">
                    ...
                    <div class="rounded-lg shadow-sm p-4" :class="theme.card"
                        v-if="brands?.some(brand => brand.models?.length)">
                        <h3 class="font-medium text-sm mb-3 flex items-center gap-1.5" :class="theme.text">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                :class="theme.icon">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                ...
                                <div class="ml-2 space-y-0.5">
                                    <div v-for="model in brand.models" :key="model.id" @click="toggleModel(model.id)"
                                        :class="[
                                            ...
                                            theme.hover,
                                            selectedModels.includes(model.id) ? 'bg-brand-blue/10 text-brand-blue' : theme.text
                                        ]">
                    ...
                    <div class="rounded-lg shadow-sm p-4" :class="theme.card">
                        ...
                        <div class="space-y-3">
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    <label class="block text-[10px] mb-0.5" :class="theme.textMuted">Min (Pkr)</label>
                                    <input type="number" placeholder="Min" v-model.number="minPrice"
                                        @input="debouncedApplyFilters"
                                        class="w-full px-2 py-1.5 border rounded focus:ring-1 focus:ring-brand-teal focus:border-brand-teal outline-none transition text-xs"
                                        :class="theme.input" />
                                </div>
                                <div>
                                    <label class="block text-[10px] mb-0.5" :class="theme.textMuted">Max (Pkr)</label>
                            ...
                            <button @click="applyFilters"
                                class="w-full font-medium py-2 rounded transition-all duration-200 shadow-sm hover:shadow text-xs"
                                :class="theme.button">
                                Apply Filter
                            </button>
                        </div>
                    ...
                    <div v-if="attributes?.filter(attr => attr.is_filterable).length" class="rounded-lg shadow-sm p-4"
                        :class="theme.card">
                        <h3 class="font-medium text-sm mb-3 flex items-center gap-1.5" :class="theme.text">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                :class="theme.icon">
                                ...
                                <div v-if="attribute.type === 'select' && attribute.options?.length" class="space-y-1">
                                    <div v-for="option in attribute.options" :key="option.id"
                                        @click="toggleAttribute(attribute.id, option.id)" :class="[
                                            'flex items-center justify-between cursor-pointer py-1 px-2 rounded transition-colors text-xs',
                                            theme.hover,
                                            isAttributeSelected(attribute.id, option.id) ? 'bg-brand-blue/10 text-brand-blue' : theme.text
                                        ]">
                    ...
                    <div v-if="activeFilterCount > 0" class="rounded-lg p-3" :class="theme.card">
                        ...
                        <button @click="resetFilters" class="mt-2 text-xs font-medium" :class="theme.textAccent">
                            Clear all filters
                        </button>
                    </div>

                    <!-- Mobile Filter Actions - Compact -->
                    <div class="lg:hidden rounded-lg shadow-sm p-3 border-t sticky bottom-0"
                        :class="[theme.card, theme.border]">
                        <div class="grid grid-cols-2 gap-2">
                            <button @click="resetFilters"
                                class="py-2 border font-medium rounded transition-colors text-xs"
                                :class="[theme.buttonOutline, theme.border]">
                                Reset All
                            </button>
                            <button @click="showMobileFilters = false"
                                class="py-2 font-medium rounded transition-colors text-xs shadow-sm"
                                :class="theme.button">
                                Show Results
                            </button>
                        </div>
                ...
                <main class="lg:col-span-2">
                    ...
                    <div class="mb-4 md:mb-5">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                            <div>
                                ...
                                <p class="text-xs md:text-sm" :class="theme.textMuted">
                                    {{ totalAds }} ads found
                                    <span v-if="selectedCategoryId" :class="theme.textAccent">
                                        in {{ selectedCategoryName }}
                                    </span>
                                    <span v-if="allLoadedAds.length > 0 && allLoadedAds.length < totalAds" class="ml-1"
                    ...
                    <div class="rounded-lg shadow-sm p-3 mb-4" :class="theme.card">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            ...
                            <div class="flex items-center justify-between sm:justify-start">
                                ...
                                <div class="sm:hidden text-xs" :class="theme.textMuted">
                            ...
                            <div class="hidden md:flex items-center space-x-3">
                                <span class="text-xs" :class="theme.textMuted">Sort:</span>
                                <SelectInput v-model="sortBy" @update:modelValue="applyFilters" placeholder="Sort By"
                                    class="min-w-[140px]">
                                    <SelectContent>
                                        <SelectItem value="newest">Newest First</SelectItem>
                            ...
                            <button @click="resetFilters" v-if="activeFilterCount > 0"
                                class="hidden md:inline-flex items-center text-xs" :class="theme.textMuted">
                    ...
                    <div v-else-if="allLoadedAds.length > 0">
                        ...
                        <div v-if="loading" class="text-center py-4">
                            <svg class="animate-spin w-6 h-6 mx-auto" :class="theme.icon" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div v-else-if="!isLoading && allLoadedAds.length === 0"
                        class="text-center py-10 md:py-12 rounded-lg shadow-sm" :class="theme.card">
                        <div class="max-w-md mx-auto px-4">
                            <svg class="w-12 h-12 md:w-16 md:h-16 mx-auto mb-3 md:mb-4" :class="theme.textMuted"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                    d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                            </svg>
                            <h3 class="text-lg md:text-xl font-semibold mb-2" :class="theme.text">
                                No matching ads found
                            </h3>
                            <p class="text-xs md:text-sm mb-4 md:mb-5" :class="theme.textMuted">
                                Try adjusting your filters to find what you're looking for.
                            </p>
                            <button @click="resetFilters"
                                class="px-5 py-2 font-medium rounded-lg transition-colors duration-200 text-xs shadow-sm"
                                :class="theme.button">
                                Reset Filters
                            </button>
                        </div>
