<template>
    <div class="mt-8 bg-white rounded-lg sm:rounded-xl shadow-sm p-4 sm:p-5 lg:p-6">
        <!-- Header -->
        ...
        <div v-if="$page.props.auth?.user" class="mb-6">
            <textarea v-model="newCommentText" placeholder="Write a comment..." rows="3"
                class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary transition resize-none"></textarea>
            <div class="flex justify-end mt-2">
                <button @click="submitComment" :disabled="!newCommentText.trim() || submitting"
                    class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark disabled:opacity-50 transition text-sm">
                    <Icon v-if="submitting" icon="lucide:loader-2" class="size-4 animate-spin inline mr-1" />
                    Post Comment
                </button>
            </div>
        </div>
        <div v-else class="mb-6 p-4 bg-gray-50 rounded-lg text-center text-sm text-gray-600">
        ...
        <div v-if="comments.length" class="space-y-4">
            <div v-for="comment in comments" :key="comment.id" class="border-b border-gray-100 last:border-0 pb-4">
                <!-- Parent comment -->
                    ...
                    <div class="flex-1">
                        ...
                        <div v-if="replyFormId === comment.id" class="mt-3">
                            <textarea v-model="replyText" placeholder="Write a reply..." rows="2"
                                class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition resize-none"></textarea>
                            <div class="flex justify-end gap-2 mt-2">
                                ...
                                <button @click="submitReply(comment.id)"
                                    ...
                                    class="px-3 py-1 bg-primary text-white rounded-lg hover:bg-primary-dark disabled:opacity-50 text-sm">
                        ...
                        <div v-if="comment.replies?.length" class="ml-6 mt-3 space-y-3">
                            <div v-for="reply in comment.replies" :key="reply.id" class="flex gap-3">
                                <div
                                    ...
                                    <Icon icon="lucide:user" class="size-3.5 text-gray-600" />
                                </div>
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-1">
...
const submitComment = () => {
    ...
    router.post(
        route("comments.store"),
        {
            ...
            parent_id: null,
            type: "text", // or pass appropriate type
        },
        {
            preserveScroll: true,
            onSuccess: () => {
