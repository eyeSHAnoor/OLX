                ...
                <div v-for="ad in allLoadedAds" :key="ad.id"
                    ...
                    <div class="relative aspect-square bg-gray-100">
                        ...
                        <div
                            ...
                            <button v-if="ad.status === 'active'" @click="markAsSold(ad)"
                                class="p-2 bg-white rounded-lg hover:bg-gray-700 hover:text-white transition-colors shadow-lg"
                                title="Mark as Sold">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M5 13l4 4L19 7" />
...
const markAsSold = async (ad: any) => {
    const confirmed = await alert.show({
        title: "Mark as Sold",
        description: `Are you sure you want to mark "${ad.ad_title}" as sold?`,
        confirmText: "Yes, Mark Sold",
        cancelText: "Cancel",
    });

    if (confirmed) {
        await updateAdStatus(ad.id, "sold");
    }
};
